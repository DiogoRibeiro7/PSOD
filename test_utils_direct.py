...
try:
    for outlier_type in ['global', 'local', 'collective', 'contextual', 'point', 'mixed']:
        ...
        n_outliers = np.sum(y)
        print(f"   ✓ Generated {outlier_type:12s} outliers: {X.shape[0]} samples, {int(n_outliers)} outliers")
...
try:
    ...
    print(f"   ✓ Warnings: {len(validation_results['warnings'])}, Errors: {len(validation_results['errors'])}")
...
try:
    ...
    print(f"   ✓ Basic metrics: precision={metrics.get('precision', 0):.3f}, recall={metrics.get('recall', 0):.3f}, f1={metrics.get('f1', 0):.3f}")

    detailed_metrics = utils.compute_detailed_metrics(y_true, y_pred, y_scores)
    print(f"   ✓ Detailed: accuracy={detailed_metrics['accuracy']:.3f}, specificity={detailed_metrics['specificity']:.3f}")
    print(f"   ✓ Confusion matrix: TP={detailed_metrics['true_positives']}, TN={detailed_metrics['true_negatives']}")
...
try:
    ...
    for strategy in ['drop', 'mean', 'median', 'mode', 'knn']:
        ...
        print(f"   ✓ {strategy:8s}: shape={df_clean.shape}, missing={df_clean.isnull().sum().sum()}")
...
try:
    ...
    print(f"   ✓ Standardized data: mean={X_scaled.select_dtypes(include=[np.number]).mean().mean():.6f}")
    print(f"   ✓ Standardized data: std={X_scaled.select_dtypes(include=[np.number]).std().mean():.6f}")
