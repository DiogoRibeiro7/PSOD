...
def plot_dataset_comparison(
    ...
    ax.set_ylabel(metric.replace('_', ' ').title(), fontsize=12)
    ax.set_title(f'Method Performance Across Datasets - {metric.upper()}',
...
def create_benchmark_dashboard(
    ...
    summary = results_df.groupby('method')['train_time'].mean().reset_index().sort_values('train_time')
    ...
    for method in results_df['method'].unique():
        ...
        ax3.scatter(data['total_time'].mean(), data['roc_auc'].mean(),
    ...
    if available_metrics:
        ...
        normalized = (summary - summary.min()) / (summary.max() - summary.min())
        sns.heatmap(normalized.T, annot=summary.T, fmt='.3f', cmap='RdYlGn',
                   center=0.5, ax=ax4, cbar_kws={'label': 'Normalized Score'})
